  ...
  return <aside className="fixed bottom-5 right-5 z-50 w-[calc(100%-2.5rem)] max-w-sm" aria-label="Help and chat">
    {open && <section className="mb-3 overflow-hidden rounded-3xl border border-blue-200 bg-white shadow-2xl shadow-slate-900/20">
      ...
      <div className="space-y-4 p-5">
        ...
        <Link href="/support" onClick={() => setOpen(false)} className="flex items-start gap-3 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-left transition hover:border-violet-400 hover:bg-violet-100">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-violet-700 text-white"><Send size={19} /></span>
          <span><strong className="block text-sm text-ink">Message our support team</strong><span className="mt-1 block text-xs leading-5 text-slate-600">Send a secure problem report when AI guidance has not resolved it.</span></span>
        </Link>
