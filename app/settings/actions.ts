...
export async function saveCompanySettings(formData: FormData) { const session = await auth(); if (!session?.user?.email) redirect("/login"); const user = await prisma.user.findUnique({ where: { email: session.user.email } }); if (!user) redirect("/login"); const name = String(formData.get("name") || "").trim(); const billingEmail = String(formData.get("billingEmail") || "").trim(); const personalName = String(formData.get("personalName") || "").trim(); const paymentMethods = String(formData.get("paymentMethods") || "STRIPE"); const country = String(formData.get("country") || "GB"); const bankAccountName = String(formData.get("bankAccountName") || "").trim(); const bankSortCode = String(formData.get("bankSortCode") || "").trim(); const bankAccountNumber = String(formData.get("bankAccountNumber") || "").trim(); const bankIban = String(formData.get("bankIban") || "").trim(); const bankBic = String(formData.get("bankBic") || "").trim(); const paymentReference = String(formData.get("paymentReference") || "").trim(); if (!name || !personalName || !["GB", "IE", "US"].includes(country)) redirect("/settings?error=name"); await prisma.company.update({ where: { id: user.companyId }, data: { name, billingEmail: billingEmail || null, country, defaultCurrency: country === "IE" ? "EUR" : country === "US" ? "USD" : "GBP", paymentMethods, bankAccountName: bankAccountName || null, bankSortCode: bankSortCode || null, bankAccountNumber: bankAccountNumber || null, bankIban: bankIban || null, bankBic: bankBic || null, paymentReference: paymentReference || null } }); await prisma.user.update({ where: { id: user.id }, data: { name: personalName } }); redirect("/settings?saved=1"); }
