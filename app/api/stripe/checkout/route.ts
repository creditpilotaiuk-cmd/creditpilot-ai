...
const catalogue: Record<string, CatalogueItem> = {
  Starter: { price: process.env.STRIPE_STARTER_PRICE_ID, type: "plan", irelandAmount: 5900, irelandLabel: "CreditPilot AI Starter" },
  Growth: { price: process.env.STRIPE_GROWTH_PRICE_ID, type: "plan", irelandAmount: 14900, irelandLabel: "CreditPilot AI Growth" },
  Professional: { price: process.env.STRIPE_PROFESSIONAL_PRICE_ID, type: "plan", irelandAmount: 28900, irelandLabel: "CreditPilot AI Professional" },
  "extra-250-invoices": { price: process.env.STRIPE_EXTRA_250_INVOICES_PRICE_ID || "price_1UD0EaLMDyY8z2tlX8rogbo8", testPrice: process.env.STRIPE_TEST_EXTRA_250_INVOICES_PRICE_ID || "price_1UD0nfLMDyY8z2tlCvZlQ9Fj", type: "addon", irelandAmount: 2900, irelandLabel: "CreditPilot AI — Extra 250 invoices" },
  "extra-1000-invoices": { price: process.env.STRIPE_EXTRA_1000_INVOICES_PRICE_ID || "price_1UD0FOLMDyY8z2tle3vKsmYU", testPrice: process.env.STRIPE_TEST_EXTRA_1000_INVOICES_PRICE_ID || "price_1UD0oLLMDyY8z2tl5jEbztTK", type: "addon", irelandAmount: 7500, irelandLabel: "CreditPilot AI — Extra 1,000 invoices" },
  "additional-growth-user": { price: process.env.STRIPE_ADDITIONAL_GROWTH_USER_PRICE_ID || "price_1UD0G8LMDyY8z2tl79Ow0vg4", testPrice: process.env.STRIPE_TEST_ADDITIONAL_GROWTH_USER_PRICE_ID || "price_1UD0p3LMDyY8z2tlNdrRHtgp", type: "addon", requiredPlan: "Growth", irelandAmount: 1400, irelandLabel: "CreditPilot AI — Additional Growth user" },
  "additional-professional-user": { price: process.env.STRIPE_ADDITIONAL_PROFESSIONAL_USER_PRICE_ID || "price_1UD0H4LMDyY8z2tlnREvyhvx", testPrice: process.env.STRIPE_TEST_ADDITIONAL_PROFESSIONAL_USER_PRICE_ID || "price_1UD0q0LMDyY8z2tliQ0w6gSZ", type: "addon", requiredPlan: "Professional", irelandAmount: 2100, irelandLabel: "CreditPilot AI — Additional Professional user" },
};
...
export async function POST(request: Request) {
  ...
  if (!irelandPricing && !price) return NextResponse.json({ error: "This membership item is not configured yet." }, { status: 400 });
  ...
  const metadata = { ...(selection.type === "plan" ? { plan: item } : { addon: item }), billingCurrency: irelandPricing ? "EUR" : "GBP", buyerType, immediateAccessConsent: buyerType === "consumer" ? "true" : "not-applicable", consentRecordedAt };
  const body = new URLSearchParams();
  const set = (key: string, value: string) => body.set(key, value);
  set("mode", "subscription");
  set("line_items[0][quantity]", "1");
  if (irelandPricing) {
    set("line_items[0][price_data][currency]", "eur");
    set("line_items[0][price_data][product_data][name]", selection.irelandLabel);
    set("line_items[0][price_data][recurring][interval]", "month");
    set("line_items[0][price_data][unit_amount]", String(selection.irelandAmount));
  } else {
