...
import { AlertTriangle, ArrowLeft, ChevronDown, Download, Gauge, Plus, PoundSterling, ShieldCheck, Users } from "lucide-react";
...
// Professional accounts display the complete customer-control workspace by default.
...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const isProfessional = user.company.plan.toUpperCase() === "PROFESSIONAL";
  const totalCustomerExposure = customers.reduce((total, customer) => total + customer.invoices.filter(invoice => !["PAID", "WRITTEN_OFF"].includes(invoice.status)).reduce((sum, invoice) => sum + Number(invoice.amount), 0), 0);
  const highRiskCustomers = customers.filter(customer => customer.riskLevel === "HIGH").length;
  const customersWithOverdueInvoices = customers.filter(customer => customer.invoices.some(invoice => invoice.status === "OVERDUE")).length;
  ...
  return <main className="flex min-h-screen"><DashboardSidebar /><div className="min-w-0 flex-1"><header className="border-b bg-white px-5 py-4 sm:px-8"><p className="text-sm text-slate-500">Credit control workspace</p><h1 className="text-xl font-bold tracking-tight text-ink">Customers</h1></header><div className="mx-auto max-w-7xl p-5 sm:p-8"><Link href="/dashboard" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-500"><ArrowLeft size={16} />Back to dashboard</Link>
    {params.creditLimit === "1" && <Notice text="Customer credit limit updated." />}{params.riskUpdated === "1" && <Notice text="Customer risk assessment refreshed." />}{params.dunning === "paused" && <Notice text="Chase workflow paused for this account." />}{params.dunning === "resumed" && <Notice text="Chase workflow resumed for this account." />}{params.error === "premium-risk" && <Warning text="External Company Checks are available on Growth and Professional plans." />}{params.error === "dunning-reason" && <Warning text="Add a reason of at least 5 characters before marking an account uncollectable." />}
    <section className="mb-7 overflow-hidden rounded-3xl bg-gradient-to-br from-[#10285f] via-[#184794] to-[#2764ff] p-6 text-white shadow-[0_22px_55px_rgba(30,64,175,0.22)] sm:p-7">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
        <div><div className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white/15"><ShieldCheck size={20} /></span><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold uppercase tracking-[0.14em]">{isProfessional ? "Professional workspace" : `${user.company.plan} workspace`}</span></div><h2 className="mt-4 text-2xl font-bold">Your customer credit-control portfolio</h2><p className="mt-2 text-sm text-blue-100">Review exposure, payment behaviour, risk evidence and workflow controls from one place.</p></div>
        <Link href="#add-customer" className="inline-flex w-fit items-center justify-center rounded-xl bg-white px-5 py-3 text-sm font-bold text-electric shadow-lg transition hover:-translate-y-0.5">Add a customer <Plus className="ml-2" size={17} /></Link>
      </div>
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <PortfolioMetric icon={Users} label="Customers" value={String(customers.length)} />
        <PortfolioMetric icon={PoundSterling} label="Open exposure" value={money(totalCustomerExposure)} />
        <PortfolioMetric icon={AlertTriangle} label="With overdue invoices" value={String(customersWithOverdueInvoices)} />
        <PortfolioMetric icon={Gauge} label="High recorded risk" value={String(highRiskCustomers)} />
      </div>
    </section>
    <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_390px]">
      <section className="overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-[0_18px_45px_rgba(30,64,175,0.10)]"><div className="flex items-center justify-between border-b border-blue-100 bg-gradient-to-r from-white to-blue-50/70 p-6"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-bold text-ink">Your customers</h2>{isProfessional && <span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-bold text-violet-700">All Professional controls available</span>}</div><p className="mt-1 text-sm text-slate-500">{customers.length} customer{customers.length === 1 ? "" : "s"} in your workspace.</p></div><Users className="text-electric" size={20} /></div>{customers.length === 0 ? <p className="p-12 text-center text-sm text-slate-500">No customers yet.</p> : <div className="divide-y">{customers.map(customer => {
        ...
        return <article key={customer.id} className="p-6 transition duration-300 hover:bg-blue-50/20"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="font-semibold text-ink">{customer.name}</p><p className="mt-1 text-sm text-slate-500">{customer.email ?? "No email added"}</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{String(externalRisk.reliability || internalRisk)}</span><a href={`/api/customers/${customer.id}/collection-pack`} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-white px-3 py-2 text-xs font-semibold text-electric hover:bg-sky"><Download size={15} />Export collection pack</a></div></div>
          ...
          <details open={isProfessional} className="group mt-5 overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-br from-slate-50 to-blue-50/50 shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between bg-white/70 p-4 text-sm font-bold text-ink transition hover:bg-blue-50"><span className="flex items-center gap-2"><Gauge size={17} className="text-electric" />Payment behaviour & workflow</span><span className="flex items-center gap-2">{isProfessional && <span className="rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-violet-700">Professional</span>}<ChevronDown size={16} className="text-slate-400 transition group-open:rotate-180" /></span></summary><div className="border-t border-blue-100 bg-white p-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Metric label="Credit Limit" value={creditLimit > 0 ? money(creditLimit) : "Not set"} /><Metric label="Current Exposure" value={money(exposure)} /><Metric label="Available Credit" value={creditLimit > 0 ? money(available) : "Not set"} /><Metric label="Average Days to Pay" value={averageDays === null ? "No payment history" : `${averageDays} days`} /><Metric label="Risk Rating" value={String(externalRisk.reliability || internalRisk)} /><Metric label="Suggested Credit Limit" value={money(suggestedLimit)} /></div>
