...
async function sendResetEmail(email: string, url: string) {
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from, to: [email], subject: "Reset your CreditPilot AI password", text: `Use this secure link to choose a new password:\n\n${url}\n\nThis link expires in 30 minutes and can only be used once.` }) });
  return response.ok;
}
