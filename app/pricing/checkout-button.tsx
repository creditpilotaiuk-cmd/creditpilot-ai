...
export function CheckoutButton({ item, label = "Subscribe to this plan", className = "button-primary w-full", businessOnly = false }: { item: string; label?: string; className?: string; businessOnly?: boolean }) {
  ...
  async function start() {
    ...
    if (businessOnly && buyerType !== "business") {
      setError("Irish memberships are currently available to business customers only.");
      return;
    }
  ...
  return <div className="space-y-3">
    <fieldset className="rounded-xl border border-slate-200 bg-white/70 p-3 text-left">
      ...
      {businessOnly ? <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">Irish memberships are currently available to business customers only while VAT treatment for consumer sales is being finalised.</p> : <label className="mt-2 flex cursor-pointer items-start gap-2 text-xs leading-5 text-slate-700"><input type="radio" name={`${fieldId}-buyer-type`} value="consumer" checked={buyerType === "consumer"} onChange={() => setBuyerType("consumer")} className="mt-1" /> <span><strong>For personal use</strong> - I am buying mainly outside my trade, business, craft or profession.</span></label>}
